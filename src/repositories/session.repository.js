...
class SessionRepository {
    ...
    async findById(sessionId) {
        return SessionModel.findById(sessionId).select("+refreshToken").lean();
    }
    ...
    async findActiveByUserId(userId) {
        return SessionModel.find({ userId, revokedAt: null, }).lean();
    }

    async touchLastActivity(sessionId) {
        return SessionModel.findByIdAndUpdate(sessionId, { lastActivityAt: new Date(), });
    }
    ...
    async revoke(sessionId, reason) {
        return SessionModel.findByIdAndUpdate(sessionId, { revokedAt: new Date(), revokeReason: reason });
    }

    async revokeAllByUserId(userId, reason) {
        return SessionModel.updateMany({ userId, revokedAt: null, },
